...
func (s *Scenario) CreatePreAuthKeyWithOptions(opts hsic.AuthKeyOptions) (*apiv1.PreAuthKey, error) {
...
// Headscale instance on behalf of the [Scenario].
func (s *Scenario) CreateUser(user string) (*apiv1.User, error) {
	if headscale, err := s.Headscale(); err == nil { //nolint:noinlineerr
...
func (s *Scenario) createHeadscaleEnvWithTags(
	...
	for _, user := range s.spec.Users {
		...
		if withURL {
			...
			var key *apiv1.PreAuthKey
			if len(preAuthKeyTags) > 0 {
				key, err = s.CreatePreAuthKeyWithTags(u.GetID().Or(0), true, false, preAuthKeyTags)
			} else {
				key, err = s.CreatePreAuthKey(u.GetID().Or(0), true, false)
			}
			...
			err = s.RunTailscaleUp(user, headscale.GetEndpoint(), key.GetKey().Or(""))
			if err != nil {
				return err
			}
		}
