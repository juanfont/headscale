...
func TestAppConnectorBasic(t *testing.T) {
	...
	taggedKey, err := scenario.CreatePreAuthKeyWithTags(
		mustParseID(userMap["user1"].Id), false, false, []string{"tag:connector"},
	)
	require.NoError(t, err)
	...
	err = connectorNode.Login(headscale.GetEndpoint(), taggedKey.Key)
	require.NoError(t, err)
...
func TestAppConnectorNonMatchingTag(t *testing.T) {
	...
	taggedKey, err := scenario.CreatePreAuthKeyWithTags(
		mustParseID(userMap["user1"].Id), false, false, []string{"tag:other"},
	)
	require.NoError(t, err)
	...
	err = otherNode.Login(headscale.GetEndpoint(), taggedKey.Key)
	require.NoError(t, err)
