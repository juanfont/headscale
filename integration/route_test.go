			...
			for _, advertiseDuringUp := range []bool{false, true} {
				...
				t.Run(name, func(t *testing.T) {
					...
					assert.EventuallyWithT(t, func(c *assert.CollectT) {
						...
						assert.NoError(c, err)
						assert.Len(c, result, 13)
					}, 30*time.Second, 200*time.Millisecond, "Verifying client can reach webservice through auto-approved route")
					...
					assert.EventuallyWithT(t, func(c *assert.CollectT) {
						...
						assertTracerouteViaIPWithCollect(c, tr, ip)
					}, 30*time.Second, 200*time.Millisecond, "Verifying traceroute goes through auto-approved router")
					...
					assert.EventuallyWithT(t, func(c *assert.CollectT) {
						...
						assert.NoError(c, err)
						assert.Len(c, result, 13)
					}, 30*time.Second, 200*time.Millisecond, "Verifying client can still reach webservice after policy change")
					...
					assert.EventuallyWithT(t, func(c *assert.CollectT) {
						...
						assertTracerouteViaIPWithCollect(c, tr, ip)
					}, 30*time.Second, 200*time.Millisecond, "Verifying traceroute still goes through router after policy change")
					...
					assert.EventuallyWithT(t, func(c *assert.CollectT) {
						...
						assert.NoError(c, err)
						assert.Len(c, result, 13)
					}, 30*time.Second, 200*time.Millisecond, "Verifying client can reach webservice after route re-approval")
					...
					assert.EventuallyWithT(t, func(c *assert.CollectT) {
						...
						assertTracerouteViaIPWithCollect(c, tr, ip)
					}, 30*time.Second, 200*time.Millisecond, "Verifying traceroute goes through router after re-approval")
...
func TestSubnetRouteACLFiltering(t *testing.T) {
	...
	assert.EventuallyWithT(t, func(c *assert.CollectT) {
		...
		assert.NoError(c, err)
		assert.Len(c, result, 13)
	}, 30*time.Second, 200*time.Millisecond, "Verifying node can reach webservice through allowed route")
	...
	assert.EventuallyWithT(t, func(c *assert.CollectT) {
		...
		assertTracerouteViaIPWithCollect(c, tr, ip)
	}, 30*time.Second, 200*time.Millisecond, "Verifying traceroute goes through router")
}
