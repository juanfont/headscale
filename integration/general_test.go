	...
	for _, target := range allClients {
		t.Run(target.Hostname(), func(t *testing.T) {
			...
			require.EventuallyWithT(t, func(c *assert.CollectT) {
				...
				for _, current := range nodes {
					if current.GetId() == node.GetId() {
						assert.False(c, current.GetOnline(), "expired node must be offline in the admin API")
						return
					}
				}
				c.Errorf("expired node %d disappeared", node.GetId())
			...
			require.EventuallyWithT(t, func(c *assert.CollectT) {
				...
				connection, found := info.ConnectedNodes[strconv.FormatUint(node.GetId(), 10)]
			...
			for _, peer := range allClients {
				...
				require.EventuallyWithT(t, func(c *assert.CollectT) {
					_, stderr, _ := peer.Execute([]string{"tailscale", "ping", node.GetName()})
			...
			_, err = headscale.Execute([]string{
				"headscale", "nodes", "expire", "--identifier", strconv.FormatUint(node.GetId(), 10), "--disable",
			})
			require.NoError(t, err)
			require.EventuallyWithT(t, func(c *assert.CollectT) {
				status, err := target.Status()
			...
			require.EventuallyWithT(t, func(c *assert.CollectT) {
				...
				for _, current := range nodes {
					if current.GetId() == node.GetId() {
						assert.True(c, current.GetOnline())
						return
					}
				}
				c.Errorf("restored node %d disappeared", node.GetId())
