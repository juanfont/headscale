...
func testEphemeralWithOptions(t *testing.T, opts ...hsic.Option) {
	...
	for _, userName := range spec.Users {
		...
		key, err := scenario.CreatePreAuthKey(user.GetID().Or(0), true, true)
		if err != nil {
...
func TestTaildrop(t *testing.T) {
	...
	user1Key, err := scenario.CreatePreAuthKey(userMap["user1"].GetID().Or(0), true, false)
	require.NoError(t, err)
	...
	for i, version := range MustTestVersions {
		...
		err = client.Login(headscale.GetEndpoint(), user1Key.GetKey().Or(""))
		require.NoError(t, err)
	...
	user2Key, err := scenario.CreatePreAuthKey(userMap["user2"].GetID().Or(0), true, false)
	require.NoError(t, err)
	...
	taggedKey, err := scenario.CreatePreAuthKeyWithTags(userMap["user1"].GetID().Or(0), true, false, []string{"tag:server"})
	require.NoError(t, err)
	...
	err = taggedClient.Login(headscale.GetEndpoint(), taggedKey.GetKey().Or(""))
	require.NoError(t, err)
...
func TestUpdateHostnameFromClient(t *testing.T) {
	...
	// verify both raw preservation ([apiv1.Node.Name]) and SaaS-matching sanitisation
	// ([apiv1.Node.GivenName]) for each awkward input.
	...
	assert.EventuallyWithT(t, func(ct *assert.CollectT) {
		...
		for _, node := range nodes {
			hostname := hostnames[strconv.FormatUint(node.GetID().Or(0), 10)]
			assert.Equal(ct, hostname, node.GetName().Or(""), "Node name should match hostname")

			// GivenName is sanitised via [dnsname.SanitizeHostname] (SaaS algorithm).
			assert.Equal(ct, dnsname.SanitizeHostname(hostname), node.GetGivenName().Or(""),
	...
	for _, node := range nodes {
		givenName := fmt.Sprintf("%d-givenname", node.GetID().Or(0))
		_, err = headscale.Execute(
			[]string{
				...
				givenName,
				"--identifier",
				strconv.FormatUint(node.GetID().Or(0), 10),
			},
		)
		require.NoError(t, err)
	}
	...
	assert.EventuallyWithT(t, func(ct *assert.CollectT) {
		...
		for _, node := range nodes {
			...
			expectedDNSNames[nodeID] = fmt.Sprintf("%d-givenname.headscale.net.", node.GetID().Or(0))
		}
	...
	assert.Eventually(t, func() bool {
		...
		for _, node := range nodes {
			hostname := hostnames[strconv.FormatUint(node.GetID().Or(0), 10)]
			givenName := fmt.Sprintf("%d-givenname", node.GetID().Or(0))
			if node.GetName().Or("") != hostname+"NEW" || node.GetGivenName().Or("") != givenName {
				return false
			}
		}
		return true
...
func TestExpireNode(t *testing.T) {
	...
	err = expiredNodeKey.UnmarshalText([]byte(node.GetNodeKey().Or("")))
	require.NoError(t, err)

	t.Logf("Node %s with node_key %s has been expired", node.GetName().Or(""), expiredNodeKey.String())
	...
	assert.EventuallyWithT(t, func(ct *assert.CollectT) {
		for _, client := range allClients {
			...
			if client.Hostname() != node.GetName().Or("") {
	...
	for _, client := range allClients {
		...
		assert.EventuallyWithT(t, func(c *assert.CollectT) {
			...
			if ok {
				...
				_, stderr, _ := client.Execute([]string{"tailscale", "ping", node.GetName().Or("")})
...
func TestSetNodeExpiryInFuture(t *testing.T) {
	...
	require.True(t, node.GetExpiry().Value.After(time.Now()))
	require.WithinDuration(t, targetExpiry, node.GetExpiry().Value, 2*time.Second)

	var nodeKey key.NodePublic
	err = nodeKey.UnmarshalText([]byte(node.GetNodeKey().Or("")))
	require.NoError(t, err)
...
func TestDisableNodeExpiry(t *testing.T) {
	...
	err = json.Unmarshal([]byte(result), &node)
	require.NoError(t, err)
	require.True(t, node.GetExpiry().IsSet(), "node should have an expiry set")
	...
	var nodeDisabled apiv1.Node
	err = json.Unmarshal([]byte(result), &nodeDisabled)
	require.NoError(t, err)

	// Expiry should be nil (or zero time) when disabled.
	if nodeDisabled.GetExpiry().IsSet() {
		require.True(t, nodeDisabled.GetExpiry().Value.IsZero(),
			"node expiry should be zero/nil after disabling")
	}

	var nodeKey key.NodePublic
	err = nodeKey.UnmarshalText([]byte(nodeDisabled.GetNodeKey().Or("")))
	require.NoError(t, err)

	// Verify peers see the node as not expired.
	for _, client := range allClients {
		if client.Hostname() == nodeDisabled.GetName().Or("") {
			continue
		}
...
func Test2118DeletingOnlineNodePanics(t *testing.T) {
	...
	assert.True(t, nodeList[0].GetOnline().Or(false))
	assert.True(t, nodeList[1].GetOnline().Or(false))
	...
	_, err = headscale.Execute(
		[]string{
			...
			fmt.Sprintf("%d", nodeList[0].GetID().Or(0)),
			"--output",
			"json",
			"--force",
		},
	...
	assert.True(t, nodeListAfter[0].GetOnline().Or(false))
	assert.Equal(t, nodeList[1].GetID().Or(0), nodeListAfter[0].GetID().Or(0))
}
