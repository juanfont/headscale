...
func TestNodeTagCommand(t *testing.T) {
	...
	// Test 1: Verify that user-owned nodes CANNOT have tags set
	// This validates the tags-as-identity model where tags and user ownership are mutually exclusive
	...
	// Attempt to set tags on user-owned node should FAIL
	_, err = headscale.Execute(
		[]string{
			"headscale",
			"nodes",
	...
	require.ErrorContains(t, err, "cannot set tags on user-owned nodes", "Setting tags on user-owned node should fail")
...
func TestTaggedNodeRegistration(t *testing.T) {
	...
	// Test 9: Verify attempting to set tags on user-owned node fails
	_, err = headscale.Execute(
		[]string{
			"headscale",
			"nodes",
	...
	require.Error(t, err, "Setting tags on user-owned node should fail")
