...
func TestTagsAuthKeyWithTagRequestDifferentTag(t *testing.T) {
	...
	if err != nil {
		...
		}, assertTimeout, 500*time.Millisecond, "checking node state")
...
func TestTagsAuthKeyWithTagNoAdvertiseFlag(t *testing.T) {
	...
	}, assertTimeout, 500*time.Millisecond, "verifying node inherited tags from auth key")
...
func TestTagsAuthKeyWithTagCannotAddViaCLI(t *testing.T) {
	...
	}, assertTimeout, 500*time.Millisecond, "waiting for initial registration")
	...
	if err != nil {
		...
		}, assertTimeout, 500*time.Millisecond, "verifying tags unchanged")
	}
}
...
func TestTagsAuthKeyWithTagAdminOverrideReauthPreserves(t *testing.T) {
	...
	}, assertTimeout, 500*time.Millisecond, "verifying admin tag assignment on server")
	...
	}, assertTimeout, 500*time.Millisecond, "verifying admin tag assignment propagated to node self")
	...
	}, assertTimeout, 500*time.Millisecond, "admin tags should be preserved after reauth on server")

	// Verify admin tags are preserved in node's self view after reauth (issue #2978)
	assert.EventuallyWithT(t, func(c *assert.CollectT) {
		assertNodeSelfHasTagsWithCollect(c, client, []string{"tag:second"})
	}, assertTimeout, 500*time.Millisecond, "admin tags should be preserved after reauth in node self")
...
func TestTagsAuthKeyWithTagCLICannotModifyAdminTags(t *testing.T) {
	...
	}, assertTimeout, 500*time.Millisecond, "admin tags should be preserved after CLI attempt on server")

	// Verify admin tags are preserved in node's self view (issue #2978)
	assert.EventuallyWithT(t, func(c *assert.CollectT) {
		assertNodeSelfHasTagsWithCollect(c, client, []string{"tag:valid-owned", "tag:second"})
	}, assertTimeout, 500*time.Millisecond, "admin tags should be preserved after CLI attempt in node self")
...
func TestTagsAuthKeyWithoutTagRegisterNoTags(t *testing.T) {
	...
	}, assertTimeout, 500*time.Millisecond, "verifying node has no tags")
...
func TestTagsAuthKeyWithoutTagCLINoOpAfterAdminWithReset(t *testing.T) {
	...
	}, assertTimeout, 500*time.Millisecond, "admin tags should be preserved after --reset on server")

	// Verify admin tags are preserved in node's self view after --reset (issue #2978)
	assert.EventuallyWithT(t, func(c *assert.CollectT) {
		assertNodeSelfHasTagsWithCollect(c, client, []string{"tag:valid-owned"})
	}, assertTimeout, 500*time.Millisecond, "admin tags should be preserved after --reset in node self")
...
func TestTagsAuthKeyWithoutTagCLINoOpAfterAdminWithEmptyAdvertise(t *testing.T) {
	...
	}, assertTimeout, 500*time.Millisecond, "admin tags should be preserved after empty --advertise-tags on server")

	// Verify admin tags are preserved in node's self view after empty --advertise-tags (issue #2978)
	assert.EventuallyWithT(t, func(c *assert.CollectT) {
		assertNodeSelfHasTagsWithCollect(c, client, []string{"tag:valid-owned"})
	}, assertTimeout, 500*time.Millisecond, "admin tags should be preserved after empty --advertise-tags in node self")
...
func TestTagsAuthKeyWithoutTagCLICannotReduceAdminMultiTag(t *testing.T) {
	...
	}, assertTimeout, 500*time.Millisecond, "admin tags should be preserved after CLI reduce attempt on server")

	// Verify admin tags are preserved in node's self view after CLI reduce attempt (issue #2978)
	assert.EventuallyWithT(t, func(c *assert.CollectT) {
		assertNodeSelfHasTagsWithCollect(c, client, []string{"tag:valid-owned", "tag:second"})
	}, assertTimeout, 500*time.Millisecond, "admin tags should be preserved after CLI reduce attempt in node self")
...
func TestTagsUserLoginOwnedTagAtRegistration(t *testing.T) {
	...
	}, assertTimeout, 500*time.Millisecond, "verifying node has advertised tag")
...
func TestTagsUserLoginNonExistentTagAtRegistration(t *testing.T) {
	...
	if err != nil {
		...
		}, assertTimeout, 500*time.Millisecond, "checking node registration result")
	}
}
...
func TestTagsUserLoginAddTagViaCLIReauth(t *testing.T) {
	...
	}, assertTimeout, 500*time.Millisecond, "checking initial tags")
	...
	}, assertTimeout, 500*time.Millisecond, "checking tags after CLI")
}
...
func TestTagsUserLoginCLINoOpAfterAdminAssignment(t *testing.T) {
	...
	}, assertTimeout, 500*time.Millisecond, "verifying admin assignment on server")

	// Verify admin assignment propagated to node's self view (issue #2978)
	assert.EventuallyWithT(t, func(c *assert.CollectT) {
		assertNodeSelfHasTagsWithCollect(c, client, []string{"tag:second"})
	}, assertTimeout, 500*time.Millisecond, "verifying admin assignment propagated to node self")
	...
	}, assertTimeout, 500*time.Millisecond, "admin tags should be preserved - CLI advertise-tags should be no-op on server")

	// Verify admin tags are preserved in node's self view after CLI attempt (issue #2978)
	assert.EventuallyWithT(t, func(c *assert.CollectT) {
		assertNodeSelfHasTagsWithCollect(c, client, []string{"tag:second"})
	}, assertTimeout, 500*time.Millisecond, "admin tags should be preserved - CLI advertise-tags should be no-op in node self")
...
func TestTagsUserLoginCLICannotRemoveAdminTags(t *testing.T) {
	...
	}, assertTimeout, 500*time.Millisecond, "admin tags should be preserved - CLI cannot remove them on server")

	// Verify admin tags are preserved in node's self view after CLI attempt (issue #2978)
	assert.EventuallyWithT(t, func(c *assert.CollectT) {
		assertNodeSelfHasTagsWithCollect(c, client, []string{"tag:valid-owned", "tag:second"})
	}, assertTimeout, 500*time.Millisecond, "admin tags should be preserved - CLI cannot remove them in node self")
...
func TestTagsAdminAPICannotSetNonExistentTag(t *testing.T) {
	...
	}, assertTimeout, 500*time.Millisecond, "waiting for registration")
...
func TestTagsAdminAPICanSetUnownedTag(t *testing.T) {
	...
	}, assertTimeout, 500*time.Millisecond, "verifying unowned tag was applied on server")
	...
	}, assertTimeout, 500*time.Millisecond, "verifying unowned tag propagated to node self")
...
func TestTagsAdminAPICannotRemoveAllTags(t *testing.T) {
	...
	}, assertTimeout, 500*time.Millisecond, "verifying original tags preserved")
}
...
func TestTagsIssue2978ReproTagReplacement(t *testing.T) {
	...
	assert.EventuallyWithT(t, func(c *assert.CollectT) {
		assertNodeSelfHasTagsWithCollect(c, client, []string{"tag:valid-owned"})
	}, assertTimeout, 500*time.Millisecond, "client should see initial tag")
	...
	}, assertTimeout, 500*time.Millisecond, "server should show tag:second after first call")
	...
	assert.EventuallyWithT(t, func(c *assert.CollectT) {
		assertNodeSelfHasTagsWithCollect(c, client, []string{"tag:second"})
	}, assertTimeout, 500*time.Millisecond, "client status.Self should update to tag:second after SECOND call")

	assert.EventuallyWithT(t, func(c *assert.CollectT) {
		assertNetmapSelfHasTagsWithCollect(c, client, []string{"tag:second"})
	}, assertTimeout, 500*time.Millisecond, "client netmap.SelfNode should update to tag:second after SECOND call")
	...
	}, assertTimeout, 500*time.Millisecond, "server should show tag:valid-unowned")
	...
	assert.EventuallyWithT(t, func(c *assert.CollectT) {
		assertNodeSelfHasTagsWithCollect(c, client, []string{"tag:valid-unowned"})
	}, assertTimeout, 500*time.Millisecond, "client status.Self should update to tag:valid-unowned after SECOND call")

	assert.EventuallyWithT(t, func(c *assert.CollectT) {
		assertNetmapSelfHasTagsWithCollect(c, client, []string{"tag:valid-unowned"})
	}, assertTimeout, 500*time.Millisecond, "client netmap.SelfNode should update to tag:valid-unowned after SECOND call")
...
func TestTagsUserLoginReauthWithEmptyTagsRemovesAllTags(t *testing.T) {
	...
	t.Run("with force-reauth", func(t *testing.T) {
		...
		}, assertTimeout, 1*time.Second, "verifying tags removed and ownership returned")
	})
}
...
func TestTagsAuthKeyConvertToUserViaCLIRegister(t *testing.T) {
	...
	}, assertTimeout, 500*time.Millisecond, "node should be tagged initially")
	...
	}, assertTimeout, 1*time.Second, "node should be user-owned after conversion via CLI register")
}
