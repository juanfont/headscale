...
func TestAuthWebFlowLogoutAndReloginSameUser(t *testing.T) {
	...
	}, assertTimeout, 2*time.Second, "validating node count matches client count after web authentication")
	...
	}, assertTimeout, 2*time.Second, "validating node persistence in database after web flow logout")
...
func TestAuthWebFlowLogoutAndReloginNewUser(t *testing.T) {
	...
	assert.EventuallyWithT(t, func(ct *assert.CollectT) {
		...
		assert.Len(ct, listNodes, len(allClients), "Expected %d nodes after web auth, got %d", len(allClients), len(listNodes))
	}, assertTimeout, 2*time.Second, "validating node count matches client count after initial web authentication")
	...
	}, assertTimeout, 2*time.Second, "validating user1 has all client nodes after web flow user switch relogin")
	...
	}, assertTimeout, 2*time.Second, "validating user2 old nodes remain in database after CLI registration to user1")
	...
	for _, client := range allClients {
		...
		}, assertTimeout, 2*time.Second, "validating %s is logged in as user1 after web flow user switch", client.Hostname())
	}
