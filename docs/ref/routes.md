...
Approve all desired routes of a subnet router by specifying it as comma separated list:
...
To accept routes advertised by a subnet router:
...
the subnet router `router` without allowing to use any service on the subnet router itself. The second ACL rule allows
the node `node` to access `service.example.net` on port tcp/80 and tcp/443 which is reachable via the subnet router.

```json title="Acces the routes of a subnet router without the subnet router itself"
{
  "hosts": {
...
The ACL snippet below defines a group `group:routers` with one member `user@`. This group owns the tag `tag:router`
which is used for `routes` in the `autoApprovers` section. The IPv4 route `192.168.0.0/24` will be automatically
...
Advertise the route `192.168.0.0/24` from a subnet routes that also advertises the tag `tag:router` when joining the tailnet:
...
The ACL snippet below defines a group `group:exits` with one member `user@`. This group owns the tag `tag:exit` which is
used for `exitNode` in the `autoApprovers` section. A new exit node which is owned by the group `group:exits` and that
also advertises the tag `tag:exit` will be automatically approved:
...
Advertise the node as exit node and also advertise the tag `tag:exit` when joining the tailnet:
