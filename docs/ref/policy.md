...
3. Start the connector node with `tailscale set --advertise-connector`. It receives the domain configuration via the
   `tailscale.com/app-connectors` capability.
4. The connector resolves a configured domain when it receives a DNS query for it, then advertises the resulting
   addresses as routes. Traffic for those addresses is then routed through the connector.

!!! warning "Current limitations"

    Headscale delivers the connector configuration, but does not yet tell clients to send DNS queries for the
    configured domains to the connector, so dynamic domain discovery does not happen on its own. Prefixes listed
    in `routes` are advertised as soon as the connector receives its configuration and work today. Tracked in
    [#1651](https://github.com/juanfont/headscale/issues/1651).
