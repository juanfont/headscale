...
type mapSession struct {
	...
	// log is the session-scoped logger with node and request context pre-attached.
	log zerolog.Logger
}
...
func (h *Headscale) newMapSession(
	...
	// Create session-scoped logger with node and request context
	sessionLog := log.With().
...
func (m *mapSession) serveLongPoll() {
	...
	m.log.Trace().Caller().Msg("Long poll session started because client connected")
	...
	defer func() {
		...
		if disconnected {
			...
			if err != nil {
				m.log.Error().Err(err).Msg("Failed to disconnect node")
			}

			m.h.Change(disconnectChanges...)
			m.afterServeLongPoll()
			m.log.Info().Str(zf.Chan, fmt.Sprintf("%p", m.ch)).Msg("node has disconnected")
		}
	}()
	...
	if err != nil {
		m.log.Error().Err(err).Msg("failed to update node from initial MapRequest")
		return
	}
	...
	m.log.Info().Str(zf.Chan, fmt.Sprintf("%p", m.ch)).Msg("node has connected")
	...
	if err := m.h.mapBatcher.AddNode(m.node.ID, m.ch, m.capVer); err != nil {
		m.log.Error().Err(err).Msg("failed to add node to batcher")
		return
	}

	m.log.Debug().Caller().Msg("AddNode succeeded in poll session")
	...
	for {
		...
		case update, ok := <-m.ch:
			...
			if err := m.writeMap(update); err != nil {
				m.log.Error().Err(err).Msg("cannot write update to client")
				return
			}
		...
		case <-m.keepAliveTicker.C:
			if err := m.writeMap(&keepAlive); err != nil {
				m.log.Error().Err(err).Msg("cannot write keep alive")
				return
			}
...
func (m *mapSession) writeMap(msg *tailcfg.MapResponse) error {
	...
	if m.isStreaming() {
		if f, ok := m.w.(http.Flusher); ok {
			...
			m.log.Error().Msg("ResponseWriter does not implement http.Flusher, cannot flush")
		}
	}
	...
	m.log.Trace().
		...
		TimeDiff("time_spent", time.Now(), startWrite).
