...
var allBatcherFunctions = []batcherTestCase{
	{"Lock", NewBatcherLockAndMapper},
	{"LockFree", NewLockFreeBatcherAndMapper},
}
...
func assertDERPMapResponse(t *testing.T, data []byte) {
	t.Helper()

	d := data[reservedResponseHeaderSize:]
...
func assertOnlineMapResponse(t *testing.T, data []byte, expected bool) {
	t.Helper()
	...
	if resp.PeersChangedPatch != nil && len(resp.PeersChangedPatch) > 0 {
		...
		assert.Equal(t, resp.PeersChangedPatch[0].Online, &expected)
		return
	}

	// Fallback to old format for backwards compatibility
	require.Len(t, resp.Peers, 1)
	assert.Equal(t, resp.Peers[0].Online, expected)
}
...
func parseUpdateAndAnalyze(data []byte) (UpdateInfo, error) {
	if len(data) < reservedResponseHeaderSize {
		return UpdateInfo{}, fmt.Errorf("data too short: %d bytes", len(data))
	}

	// Extract the JSON payload
	jsonData := data[reservedResponseHeaderSize:]

	// Check if data is compressed (zstd compression starts with specific bytes)
	if len(jsonData) > 4 && jsonData[0] == 0x28 && jsonData[1] == 0xb5 && jsonData[2] == 0x2f && jsonData[3] == 0xfd {
		// Decompress zstd data
		decompressed, err := zstdframe.AppendDecode(nil, jsonData)
		if err != nil {
			return UpdateInfo{}, fmt.Errorf("zstd decompression failed: %v", err)
		}
	...
	if err := json.Unmarshal(jsonData, &resp); err != nil {
		return UpdateInfo{}, fmt.Errorf("invalid JSON (len=%d): %v", len(jsonData), err)
	}
...
func validateUpdateContent(data []byte) (bool, string) {
	...
	// Check if data is compressed (zstd compression starts with specific bytes)
	if len(jsonData) > 4 && jsonData[0] == 0x28 && jsonData[1] == 0xb5 && jsonData[2] == 0x2f && jsonData[3] == 0xfd {
		// Decompress zstd data to analyze the actual content
		decompressed, err := zstdframe.AppendDecode(nil, jsonData)
		if err != nil {
			return false, fmt.Sprintf("zstd decompression failed: %v", err)
		}
		jsonData = decompressed
	}
...
func TestEnhancedNodeTracking(t *testing.T) {
	...
	// Marshal to JSON and add header
	jsonData, err := json.Marshal(resp)
	require.NoError(t, err)

	data := make([]byte, 4)
	binary.LittleEndian.PutUint32(data, uint32(len(jsonData)))
...
func TestBatcherBasicOperations(t *testing.T) {
	for _, batcherFunc := range allBatcherFunctions {
		t.Run(batcherFunc.name, func(t *testing.T) {
			...
			// Second node should not have any updates
			assert.Len(t, tn2.ch, 0)
...
func TestBatcherUpdateTypes(t *testing.T) {
	for _, batcherFunc := range allBatcherFunctions {
		t.Run(batcherFunc.name, func(t *testing.T) {
			...
			batcher.AddNode(testNodes[0].n.ID, ch, "zstd", tailcfg.CapabilityVersion(100))
				...
				case data := <-ch:
					...
					if len(data) > 0 {
						if valid, reason := validateUpdateContent(data); valid {
							t.Logf("Update %d: %d bytes, valid", updateCount, len(data))
						} else {
							t.Logf("Update %d: %d bytes, invalid: %s", updateCount, len(data), reason)
						}
					} else {
						t.Logf("Update %d: empty update", updateCount)
					}
				case <-timeout:
					// Expected: 5 changes should generate 5 updates (no batching in current implementation)
					expectedUpdates := 5
	...
	for _, batcherFunc := range allBatcherFunctions {
		t.Run(batcherFunc.name, func(t *testing.T) {
			...
			for i := range 100 {
				...
				go func() {
					defer wg.Done()
					batcher.AddNode(testNode.n.ID, ch1, "zstd", tailcfg.CapabilityVersion(100))
				}()
				...
				go func() {
					...
					batcher.AddNode(testNode.n.ID, ch2, "zstd", tailcfg.CapabilityVersion(100))
				}()
		...
		t.Run(batcherFunc.name, func(t *testing.T) {
			...
			for i := range 50 {
				func() {
					...
					batcher.AddNode(testNode.n.ID, ch, "zstd", tailcfg.CapabilityVersion(100))
					batcher.AddWork(change.DERPSet)
	...
	for _, batcherFunc := range allBatcherFunctions {
		t.Run(batcherFunc.name, func(t *testing.T) {
			...
			for _, node := range stableNodes {
				...
				batcher.AddNode(node.n.ID, ch, "zstd", tailcfg.CapabilityVersion(100))
				...
				go func(nodeID types.NodeID, channel chan []byte) {
					for {
						...
						case data := <-channel:
							if valid, reason := validateUpdateContent(data); valid {
								tracker.recordUpdate(nodeID, len(data))
							} else {
			...
			churningChannels := make(map[types.NodeID]chan []byte)
...
func TestBatcherScalability(t *testing.T) {
	...
	nodes := []int{25, 50, 100} // grinds to a halt because of other bottlenecks.
	// 250, 500, 1000,
						...
						for cycle := 0; cycle < tc.cycles; cycle++ {
							...
							for i, node := range chaosNodes {
								...
								if tc.chaosType == "connection" || tc.chaosType == "mixed" {
									...
									// Then reconnection
									go func(nodeID types.NodeID, channel chan []byte, index int) {
										defer func() {
											if r := recover(); r != nil {
												atomic.AddInt64(&panicCount, 1)
											}
										...
										batcher.AddNode(nodeID, channel, "", tailcfg.CapabilityVersion(100))
										connectedNodesMutex.Lock()
	...
	for _, batcherFunc := range allBatcherFunctions {
		t.Run(batcherFunc.name, func(t *testing.T) {
			...
			for i := 0; i < len(allNodes); i++ {
				...
				for updateNum := 0; updateNum < 10; updateNum++ {
					select {
					case data := <-allNodes[i].ch:
						nodeUpdates++
						totalUpdates++

						// Parse and examine the update
						if len(data) < reservedResponseHeaderSize {
							t.Errorf("Node %d update %d: data too short", i, updateNum)
							continue
						}
						...
						var resp tailcfg.MapResponse
						if err := json.Unmarshal(jsonData, &resp); err != nil {
							t.Errorf("Node %d update %d: invalid JSON: %v", i, updateNum, err)
							continue
						}
						...
						} else if len(resp.PeersChangedPatch) > 0 {
							updateType = "PATCH"
						} else if resp.DERPMap != nil {
							updateType = "DERP"
						}

						t.Logf("  Update %d: %s - Peers=%d, PeersChangedPatch=%d, DERPMap=%v",
							updateNum, updateType, len(resp.Peers), len(resp.PeersChangedPatch), resp.DERPMap != nil)

						if len(resp.Peers) > 0 {
							t.Logf("    Full peer list with %d peers", len(resp.Peers))
							for j, peer := range resp.Peers[:min(3, len(resp.Peers))] {
								t.Logf("      Peer %d: NodeID=%d, Online=%v", j, peer.ID, peer.Online)
							}
						}
						if len(resp.PeersChangedPatch) > 0 {
							t.Logf("    Patch update with %d changes", len(resp.PeersChangedPatch))
							for j, patch := range resp.PeersChangedPatch[:min(3, len(resp.PeersChangedPatch))] {
								t.Logf("      Patch %d: NodeID=%d, Online=%v", j, patch.NodeID, patch.Online)
							}
						}

					case <-time.After(500 * time.Millisecond):
						// No more updates for this node within timeout
						break
					}
				}
			...
			case data := <-nodes[0].ch:
				...
				if len(data) >= reservedResponseHeaderSize {
					...
					if err := json.Unmarshal(jsonData, &resp); err == nil {
						...
						t.Logf("  Peers: %d", len(resp.Peers))
						t.Logf("  PeersChangedPatch: %d", len(resp.PeersChangedPatch))
						t.Logf("  PeersChanged: %d", len(resp.PeersChanged))
						t.Logf("  PeersRemoved: %d", len(resp.PeersRemoved))
						t.Logf("  DERPMap: %v", resp.DERPMap != nil)
						t.Logf("  KeepAlive: %v", resp.KeepAlive)
						t.Logf("  Node: %v", resp.Node != nil)

						if len(resp.Peers) > 0 {
							t.Logf("SUCCESS: Full peer list received with %d peers", len(resp.Peers))
						} else if len(resp.PeersChangedPatch) > 0 {
						...
						if err != nil {
							...
							if len(peers) > 0 && len(resp.Peers) == 0 {
					...
					t.Errorf("Response data too short: %d bytes", len(data))
				}
