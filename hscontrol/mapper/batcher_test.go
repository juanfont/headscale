	...
	for _, batcherFunc := range allBatcherFunctions {
		t.Run(batcherFunc.name, func(t *testing.T) {
			...
			for i := range 100 {
				...
				go func() {
					defer wg.Done()
					batcher.AddNode(testNode.n.ID, ch1, false, "zstd", tailcfg.CapabilityVersion(100))
				}()
				...
				go func() {
					...
					batcher.AddNode(testNode.n.ID, ch2, false, "zstd", tailcfg.CapabilityVersion(100))
				}()
		...
		t.Run(batcherFunc.name, func(t *testing.T) {
			...
			for i := range 50 {
				func() {
					...
					batcher.AddNode(testNode.n.ID, ch, false, "zstd", tailcfg.CapabilityVersion(100))
					batcher.AddWork(change.DERPSet)
	...
	for _, batcherFunc := range allBatcherFunctions {
		t.Run(batcherFunc.name, func(t *testing.T) {
			...
			for _, node := range stableNodes {
				...
				batcher.AddNode(node.n.ID, ch, false, "zstd", tailcfg.CapabilityVersion(100))
				...
				for i := range numCycles {
					for _, node := range churningNodes {
						...
						go func(nodeID types.NodeID) {
							...
							batcher.AddNode(nodeID, ch, false, "zstd", tailcfg.CapabilityVersion(100))
