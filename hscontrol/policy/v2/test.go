	...
	for _, result := range r.Results {
		if !result.Passed {
			for _, e := range result.Errors {
				errs = append(errs, fmt.Sprintf("%s: %s", result.Src, e))
			}

			for _, dest := range result.AcceptFail {
				errs = append(errs, fmt.Sprintf("%s -> %s: expected ALLOWED, got DENIED", result.Src, dest))
			}

			for _, dest := range result.DenyFail {
				errs = append(errs, fmt.Sprintf("%s -> %s: expected DENIED, got ALLOWED", result.Src, dest))
			}
		}
	}
...
func (pm *PolicyManager) checkMatcherAccess(srcIPs, destIPs *netipx.IPSet, ports []tailcfg.PortRange, proto Protocol) bool {
	...
	for _, rule := range pm.filter {
		// Check if any source IP matches the rule's source IPs
		srcMatches := false
		...
		if len(rule.IPProto) > 0 {
			requestedProtos, _ := proto.parseProtocol()
			protoMatches := false
