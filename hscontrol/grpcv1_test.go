...
// TestSetTags_UserXORTags tests the critical validation that prevents setting tags
// on user-owned nodes, enforcing the tags-as-identity model where tags and users
// are mutually exclusive ownership mechanisms.
func TestSetTags_UserXORTags(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		{
			name:           "reject setting tags on user-owned node",
			nodeID:         uint64(userOwnedNode.ID()),
			tags:           []string{"tag:server"},
			wantErr:        true,
			wantCode:       codes.InvalidArgument,
			wantErrMessage: "cannot set tags on user-owned nodes",
		},
		{
			name:           "reject multiple tags on user-owned node",
			nodeID:         uint64(userOwnedNode.ID()),
