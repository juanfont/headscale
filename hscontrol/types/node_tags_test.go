...
func TestNodeIsTagged(t *testing.T) {
	tests := []struct {
		...
		{
			name: "node registered with tagged authkey - is tagged",
			node: Node{
				AuthKey: &PreAuthKey{
...
func TestNodeViewIsTagged(t *testing.T) {
	tests := []struct {
		...
		{
			name: "tagged node via AuthKey",
			node: Node{
				AuthKey: &PreAuthKey{
					Tags: []string{"tag:web"},
				},
			},
			want: true, // Node IS tagged, even though AuthKey not exported in view
		},
		{
...
func TestNodeHasTag(t *testing.T) {
	tests := []struct {
		...
		{
			name: "node has tag in both Tags and AuthKey",
			node: Node{
				Tags: []string{"tag:server"},
				AuthKey: &PreAuthKey{
					Tags: []string{"tag:database"},
...
func TestNodeOwnershipModel(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			wantIsTagged: true,
			description:  "Node registered with tagged authkey is tagged",
		},
	}
