...
// SplitResolvers returns a map of domain to DNS resolvers and a list of search domains.
// If a nameserver is a valid IP, it will be used as a regular resolver.
// If a nameserver is a valid URL, it will be used as a DoH resolver.
// If a nameserver is neither a valid URL nor a valid IP, it will be ignored.
func (d *DNSConfig) SplitResolvers() (map[string][]*dnstype.Resolver, []string) {
...
func (d *deprecator) warnWithAlias(newKey, oldKey string) {
	...
	if viper.IsSet(oldKey) {
		d.warns = append(d.warns, fmt.Sprintf("The %q configuration key is deprecated. Please use %q instead. %q will be removed in the future.", oldKey, newKey, oldKey))
	}
}
...
func (d *deprecator) fatal(newKey, oldKey string) {
	if viper.IsSet(oldKey) {
		d.fatals = append(d.fatals, fmt.Sprintf("The %q configuration key is deprecated. Please use %q instead. %q has been removed.", oldKey, newKey, oldKey))
	}
}
...
func (d *deprecator) fatalIfNewKeyIsNotUsed(newKey, oldKey string) {
	if viper.IsSet(oldKey) && !viper.IsSet(newKey) {
		...
		d.warns = append(d.warns, fmt.Sprintf("The %q configuration key is deprecated. Please use %q instead. %q has been removed.", oldKey, newKey, oldKey))
	}
}
...
func (d *deprecator) warn(oldKey string) {
	if viper.IsSet(oldKey) {
		d.warns = append(d.warns, fmt.Sprintf("The %q configuration key is deprecated and has been removed. Please see the changelog for more details.", oldKey))
	}
}
