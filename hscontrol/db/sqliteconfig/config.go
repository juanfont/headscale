...
func Default(path string) *Config {
	return &Config{
		Path:              path,
		BusyTimeout:       DefaultBusyTimeout,    // 10s timeout for concurrent access
		JournalMode:       JournalModeWAL,        // Best concurrency + crash recovery
		AutoVacuum:        AutoVacuumIncremental, // Gradual space reclamation
		WALAutocheckpoint: 0,                     // Disabled for better control
		Synchronous:       SynchronousNormal,     // Balanced safety/performance
		ForeignKeys:       true,                  // Data integrity enforcement
	}
}
...
func Memory() *Config {
	return &Config{
		...
		ForeignKeys:       true,
		// Other settings use zero values (driver defaults)
	}
}
