...
func TestConfigToURL(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			name:   "default config",
			config: Default("/path/to/db.sqlite"),
			want:   "file:/path/to/db.sqlite?_pragma=busy_timeout=10000&_pragma=journal_mode=WAL&_pragma=auto_vacuum=INCREMENTAL&_pragma=wal_autocheckpoint=0&_pragma=synchronous=NORMAL&_pragma=foreign_keys=ON",
		},
		{
			name:   "memory config",
			config: Memory(),
