...
func TestBuildPostgresDSN(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			want: `host='localhost' dbname='headscale' user='hs' ` +
				`sslmode=disable port=5432 password='secret'`,
		},
		{
			name: "ssl true leaves libpq default",
			cfg:  types.PostgresConfig{Host: "db", Name: "headscale", User: "hs", Ssl: "true"},
			want: `host='db' dbname='headscale' user='hs'`,
		},
		{
			...
			want: `host='db' dbname='headscale' user='hs' sslmode='verify-full'`,
		},
		{
			// Regression: spaces, quotes and backslashes used to be
			// concatenated raw, truncating the DSN or injecting parameters.
			name: "password with space quote and backslash is quoted",
			cfg: types.PostgresConfig{
				Host: "db",
				Name: "headscale",
				User: "hs",
		...
		{
			name: "unix socket host is quoted",
			cfg: types.PostgresConfig{
				Host: "/var/run/postgresql",
				Name: "headscale",
				User: "hs",
			},
			want: `host='/var/run/postgresql' dbname='headscale' user='hs'`,
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			assert.Equal(t, tt.want, postgresDSN(tt.cfg))
		})
	}
}
